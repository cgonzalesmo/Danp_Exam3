...
class MainActivity : AppCompatActivity() {
    ...
    @SuppressLint("UnsafeOptInUsageError")
    private fun Slider.setup1(camera: Camera) {
        camera.cameraInfo.exposureState.let {
            value = it.exposureCompensationIndex.toFloat()
            valueFrom = it.exposureCompensationRange.lower.toFloat()
            valueTo = it.exposureCompensationRange.upper.toFloat()
    ...
    private fun bindCameraUseCases() {
        ...
        try {
            ...
            val simpleOnScaleGestureListener: SimpleOnScaleGestureListener =
                object : SimpleOnScaleGestureListener() {
                    override fun onScale(detector: ScaleGestureDetector): Boolean {
                        val currentZoomRatio = camera!!.cameraInfo
            ...
            val scaleGestureDetector = ScaleGestureDetector(
                this.applicationContext, simpleOnScaleGestureListener
            )
            viewFinder.setOnTouchListener(OnTouchListener { view, motionEvent ->
